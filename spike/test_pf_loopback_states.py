...
@pytest.mark.parametrize('family,address', [(2,'127.0.0.1'),(30,'::1')])
...
def test_selector_matches_only_tcp_loopback_and_one_port(family,address,reverse):
    ...
    addr=socket.inet_pton(socket.AF_INET6 if ":" in address else socket.AF_INET,address)
...
@pytest.mark.parametrize('output', ['', HEADER, HEADER+'TST=ESTABLISHED\n',
    HEADER+'TST=SYN_SENT\n', HEADER+'TST=LISTEN\n'])
def test_live_or_unproven_snapshot_never_opens_pf(output):
    ...
    assert not pf_adapter.clear_inactive_loopback_proxy_states(
        lambda *a:SimpleNamespace(returncode=0,stdout=output,stderr=""),1080,opener=forbidden)
...
def test_no_global_pf_flush_and_preserve_other_ports():
    ...
    output=HEADER+'TST=CLOSED\n'
    assert pf_adapter.clear_inactive_loopback_proxy_states(
        lambda *a:SimpleNamespace(returncode=0,stdout=output,stderr=""),1080,opener=opener,ioctl_fn=ioctl)
    assert len(calls)==4
...
def test_local_proxy_rules_are_bidirectional_without_reply_to_before_reply_to():
    rules=tproxy.PF_RULES.format(port=1080)
    before=rules[:rules.index('reply-to')]
    for host in ('127.0.0.1','::1'):
        assert f'from {host} to {host} port 1080 flags S/SA keep state' in before
        assert f'from {host} port 1080 to {host} flags S/SA keep state' in before
