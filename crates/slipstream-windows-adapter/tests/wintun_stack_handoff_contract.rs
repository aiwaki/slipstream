...
const CONTRACT: &str = include_str!("../../../contracts/windows-wintun-stack-handoff-v1.json");
...
fn contract() -> ContractFixture {
    serde_json::from_str(CONTRACT).expect("Wintun-to-stack handoff v1 must be valid JSON")
}

#[test]
fn contract_freezes_the_disposable_native_handoff() {
    let fixture = contract();
    assert_eq!(fixture.schema_version, 1);
    assert_eq!(fixture.contract, "slipstream.windows_wintun_stack_handoff");
    assert_eq!(fixture.contract_version, 1);
    assert_eq!(fixture.scope["transport"], "ipv4_udp");
...
#[test]
fn selected_stack_boundary_is_bounded_and_adapter_dependency_is_dev_only() {
    for required in [
        ...
        assert!(
            STACK_MODULE.contains(required),
            "missing selected-stack boundary {required}"
        );
    }
...
#[test]
fn native_fixture_uses_stack_output_instead_of_a_manual_response_packet() {
    let start = WINTUN_FIXTURE
        .find("fn prove_ipv4_packet_round_trip(")
        .expect("missing IPv4 Wintun packet proof");
    let end = WINTUN_FIXTURE[start..]
        .find("fn prove_ipv6_packet_round_trip(")
        .map(|offset| start + offset)
        .expect("missing end of IPv4 Wintun packet proof");
    let proof = &WINTUN_FIXTURE[start..end];
    for required in [
        "RawPacketUdpStackV1::new_ipv4",
        ...
        assert!(
            ...
            "missing Wintun handoff {required}"
        );
    }
    assert!(
        !proof.contains("build_ipv4_udp_packet("),
        "the selected-stack handoff cannot synthesize its response manually"
    );
}

#[test]
fn production_host_remains_closed_and_native_workflow_executes_exactly() {
    let fixture = contract();
    ...
    assert_eq!(
        fixture.remaining_gates["ipv6_udp_wintun_stack_handoff"],
        "required"
    );
    assert_eq!(
    ...
    for production_source in [
        ...
        assert!(!production_source.contains("RawPacketUdpStackV1"));
    }
    assert_eq!(
        WORKFLOW
            .matches("contracts/windows-wintun-stack-handoff-v1.json")
            .count(),
        2
    );
    assert_eq!(
        WORKFLOW
            .matches("Qualify Wintun-to-selected-stack IPv4 UDP handoff")
            .count(),
        1
    );
    assert_eq!(
        WORKFLOW
            .matches("-TestName native_wintun_ipv4_udp_round_trip_crosses_the_selected_stack")
            .count(),
        1
    );
}
