...
pub fn reduce_semantic_route_signal(
    ...
    if !signal.top_level {
        return decision(signal, false, None, NotTopLevel);
    }
    if signal.confidence_bps < MIN_SEMANTIC_SIGNAL_CONFIDENCE_BPS {
        return decision(signal, false, None, LowConfidence);
    }
    if signal.observed_at_unix_ms
        > context
            .now_unix_ms
            .saturating_add(MAX_SEMANTIC_SIGNAL_FUTURE_SKEW_MS)
    {
        return decision(signal, false, None, Future);
    }
    if context
        .now_unix_ms
        .saturating_sub(signal.observed_at_unix_ms)
        > MAX_SEMANTIC_SIGNAL_AGE_MS
    {
        return decision(signal, false, None, Stale);
    }
    if context.signal_id_seen {
        return decision(signal, false, None, Replay);
    }
    if matches!(
        context.route_class,
        RouteClass::DirectPassthrough | RouteClass::DirectFirst | RouteClass::LocalBypass
    ) {
        return decision(signal, false, None, ProtectedRoute);
    }
    if context.route_class == RouteClass::GeoExit {
        return decision(signal, false, None, AlreadyGeoExit);
    }
    if context.route_class != RouteClass::Unknown {
        return decision(signal, false, None, ProtectedRoute);
    }
    if !context.owned_geph_payload_ready {
        return decision(signal, false, None, BackendUnavailable);
    }
    if context
        .last_accepted_at_unix_ms
        .is_some_and(|last| context.now_unix_ms.saturating_sub(last) < SEMANTIC_SIGNAL_COOLDOWN_MS)
    {
        return decision(signal, false, None, RateLimited);
    }
    decision(signal, true, ConfirmExactHostGeoExit, Accepted)
}
