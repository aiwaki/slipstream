...
|---|---|---|---|---|
| 2026-07-27 | Chrome for Testing GUI bootstrap namespace | Test-composition defect isolated by protected run `30265636270`; narrow follow-up in progress | The exact-main run passed owned-Geph payload, trayless continuity, LaunchAgent recovery, and system-path cleanup, but Chrome's network service repeatedly logged a Mach rendezvous `bootstrap_look_up` failure with `Permission denied (1100)` and never requested the fixture. Dropping to the console user's POSIX credentials did not enter that user's GUI bootstrap namespace. The follow-up prefixes the exact browser command with `launchctl asuser <same-console-uid>` while preserving the existing UID/GID/groups, fresh profile, dedicated process group, and fail-closed cleanup. It changes no route, extension detector, PF/DNS/proxy/PAC/VPN state, or production distribution assumption. | Pass ordinary PR and exact-main gates, then require the same learned exact-host route, one reload, complete CSS/JavaScript/image delivery, DOM marker, browser-ready callback, and cleanup in a new protected exact-main run. |
