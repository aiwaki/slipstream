...
|---|---|---|---|---|
| 2026-07-27 | Chrome for Testing GUI bootstrap and LaunchServices | Direct LaunchAgent execution rejected; sandboxed LaunchServices composition in progress | Protected runs `30270341870`, `30279476090`, and `30282319977` separately rejected root/manual identity drop, an exact `gui/<uid>` Aqua LaunchAgent, and the same LaunchAgent with `SessionCreate=true`, all with Mach rendezvous `1100`. PR #236 and exact-main CI `30284480903` passed, but protected run `30284938688` proved that explicit `--no-sandbox` only changed the direct-launch failure to `Unknown service name (1102)`. The independent owned-Geph lifecycle still passed initial, trayless, and recovered real TLS payload and restored system state, so the remaining failure is browser bootstrap composition. The follow-up removes the sandbox opt-out and launches Chrome through LaunchServices from the exact Aqua user job. Admission requires expected UID, bundle executable family, and unique profile; cleanup retains the proven browser process group for same-bundle helpers, revalidates identity before signaling, captures both launcher and browser stderr, and proves exact launcher and browser absence. A partial asynchronous bootstrap or a timed-out browser admission additionally requires a bounded stable post-bootout absence window. Codebase-memory MCP again returned `Transport closed`, so discovery used the documented narrow `rtk` fallback. | Pass ordinary PR and exact-main gates, then require sandboxed learned exact-host routing, one reload, complete CSS/JavaScript/image delivery, DOM marker, browser-ready callback, and exact cleanup in protected CI. |
