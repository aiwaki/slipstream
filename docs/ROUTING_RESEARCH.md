...
| 2026-07-15 | Packaged lifecycle CI `EPERM` without operation context | Instrumented; repeated qualification passed | An earlier PR #107 run and the first two packaged runs for PR #109 ended with a bare `[Errno 1] Operation not permitted` after the private anchor became active, while adjacent `main` runs were green. Stage instrumentation later localized one post-merge occurrence to the third clean-profile Chrome probe at `after-tray-crash:chrome`; the same merge commit passed on its immediate rerun, including tray crash/restart, fresh HTTPS/Chrome/Safari clients, wake/network re-arm, sentinel preservation, and uninstall. This is evidence of an intermittent Chrome harness/runner edge, not a routing or PF diagnosis. Chrome OS operations now report whether the denial occurred during executable preflight, user identity lookup, profile ownership, spawn, process-group verification/stop, or capture I/O; no retry or `EPERM` suppression was added. | Preserve the named failing operation before changing only that owned CI code. Do not alter routing, permissions, or PF behavior in response to a Chrome harness `EPERM`. |
