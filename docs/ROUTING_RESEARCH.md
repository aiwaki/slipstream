...
|---|---|---|---|---|
| 2026-07-29 | Disposable Wintun-to-selected-stack IPv6 UDP handoff | Implemented locally; native evidence pending; no workstation or production-host mutation | A new isolated `raw_packet_udp_ipv6_v1` boundary leaves frozen IPv4 v1 unchanged while reusing its structured exchange result and error taxonomy. Fixed MTU, queues, burst, poll steps, one request/response, mandatory IPv6 UDP checksum validation, deterministic same-seed output, and fail-closed size/configuration checks are covered by five focused tests. The existing disposable real OS socket and exact `/128` Wintun route now retain the captured Layer 3 packet, pass it through the selected stack, inject only the stack-emitted response, and reject any retained manual IPv6 response builder. A separate contract keeps the dependency dev-only and production host, default route, DNS, external proxy/PAC/VPN, external routes, and Discord/YouTube Geph behavior closed. Local stack tests (`35`), Windows adapter tests (`211`), and clippy passed. | Run the native workflow on AMD64 and ARM64. Merge only after exact packet handoff plus owned route/address/session/adapter cleanup pass; keep TCP and production composition separate. |
| 2026-07-29 | Disposable Wintun-to-selected-stack IPv4 UDP handoff | Merged and exact-main green; no workstation or production-host mutation | PR #263 merged as `105551ec27f8139e455783b7ae2bf89d63812166`; exact-main native AMD64/ARM64 run `30472082661`, CI `30472083693`, and audit `30472083055` passed. The existing real Windows UDP socket and exact owned Wintun route retain the exact captured Layer 3 packet and pass it through a reusable bounded `smoltcp 0.13.1` boundary. Only the stack-emitted response packet is injected back into Wintun, so successful receipt by the real OS socket proves checksums and the native return path. Fixed MTU, queues, poll steps, one request/response, deterministic input, and structured errors keep the handoff fail-closed. The stack remains a dev-only qualification dependency; source and contracts reject production service-host composition, default-route/DNS/external proxy/PAC/VPN mutation, external route ownership, and every Discord/YouTube Geph edge. | Add the separate IPv6 UDP Wintun-to-stack handoff without changing frozen IPv4 v1 or composing the production service host; keep TCP and production composition as later independent gates. |
