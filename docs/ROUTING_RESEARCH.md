...
|---|---|---|---|---|
| 2026-07-25 | Generic Safari page stalls loop between system destination and Xbox DNS | Root cause reproduced; bounded local recovery fix under qualification | The exact merged YouTube artifact passed real `yt-dlp --test` media delivery, but `crystalidea.com` completed over HTTP/1.1 and repeatedly stalled over HTTP/2 after 16,366 bytes. After clean uninstall, the unchanged system path also stalled: HTTP/2 delivered zero bytes and HTTP/1.1 stopped after 16,120 of 21,726 bytes. Slipstream therefore did not create the underlying reachability problem, but its generic recovery state machine could not finish recovery: a failed Xbox-DNS stream cleared its candidate marker and sent the next request back to the original system destination. Generic unknown traffic now advances through a bounded `system destination -> app-owned Xbox DNS -> local DoH/strategy ladder` sequence. A failed local strategy remains on that ladder until the ten-minute recovery window expires. Protected route classes remain excluded and no stage may select Geph. | Pass full tests and packaged lifecycle, install only the exact reviewed artifact, then require ChatGPT, Discord, YouTube media, and repeated HTTP/2 page smoke before leaving it active. |
...
| 2026-07-25 | YouTube page succeeds while media TLS fails under installed Slipstream | Reproduced on the exact-main packaged app; policy fix under qualification | The workstation smoke reached YouTube control through local desync, then ten actual `yt-dlp --test` media attempts failed inside transparent handling with `SSL: UNEXPECTED_EOF_WHILE_READING` or `SSLV3_ALERT_UNEXPECTED_MESSAGE`. After the prepared one-shot uninstall, the exact captured `*.googlevideo.com` URL returned HTTP `206` and 65,536 bytes in 0.142 seconds over the unchanged system route. The previous bundled policy forced every Googlevideo host through fake ClientHello strategies. Split the protected YouTube family: `youtube.com` and control/static families remain `local_bypass/fake_only`; `googlevideo.com` is exact `direct_passthrough/direct`. Both remain prohibited from Geph, Smart DNS promotion, and external-state mutation. | Require shared Python/Rust/Windows vectors, packaged read-only classification checks, full CI, then repeat the exact-artifact media smoke with bounded rollback before leaving Slipstream installed. |
