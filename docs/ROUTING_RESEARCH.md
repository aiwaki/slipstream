...
|---|---|---|---|---|
| 2026-07-15 | Non-transparent connection-race socket adapter | Implemented behind an unused runtime boundary | `spike/connection_race_io.py` translates the pure reducer's resolve/start/cancel/wake commands into owned async tasks. It preserves resolver order with stable address deduplication, connects only to numeric IPv4/IPv6 candidates, gives the caller only the winning stream, and closes late, losing, timed-out, or caller-cancelled streams. Real tests use loopback sockets plus deterministic resolver/connector failures; open circuits and Discord-to-Geph mismatches reject before any I/O. The adapter does not import or modify `tproxy.py`, PF, launchd, system DNS, proxy, PAC, or VPN state. | Keep live daemon wiring out of this PR. Define the explicit non-transparent call site and lifecycle boundary separately, then qualify it with fake endpoints before any disposable installed test. |
| 2026-07-15 | Packaged lifecycle CI `EPERM` without stage context | Diagnostic gap; rerun passed | The first packaged lifecycle run for PR #107 ended with `[Errno 1] Operation not permitted`, while its rerun passed. The emitted failure bundled daemon/PF log context but did not identify the active harness stage, so the permission error cannot yet be assigned safely to install, browser, PF, or cleanup behavior. A green rerun is useful evidence but does not explain the first failure. | Add stage identity to lifecycle exceptions before changing routing or permission behavior in response to a recurrence. Preserve the failing run when it happens again. |
