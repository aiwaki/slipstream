...
| 2026-07-12 | Partial local stream stall after clean client EOF | Fixed in this PR | A local TLS route can return an initial payload above the former 8 KiB success threshold and then stop. On the affected network, `crystalidea.com` returned HTTP 200 with 16,366 of 21,726 bytes before a 25 s client timeout. The prior recovery saw only abnormal client transport aborts, so a normal client EOF after the timeout never scheduled the local DNS retry. A single clean EOF remains ambiguous: only two client-first EOFs after at least 15 s without downstream progress, for the same unknown host in five minutes, demote the exact strategy. The next retry makes an app-owned RFC 8484 Xbox DNS query and tries its answer locally with plain TLS. That retry also needs two matching clean-EOF signals before it is discarded; one ordinary keep-alive close is not enough. Server-first EOF clears pending evidence. This path never selects or learns Geph. | Keep the bounded repeated-EOF and protected-host regression tests; validate the next exact-host retry against a packaged app. |
