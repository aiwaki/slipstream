...
|---|---|---|---|---|
| 2026-07-11 | Anchor-scoped `pfctl -F all` | Fixed; privileged smoke pending | macOS documents `-F all` as including the shared state table. An anchor argument scopes rulesets, but it does not make a global state flush acceptable. Flush only `rules` and `nat` in `com.apple/slipstream`; forbid `all` and `states` in daemon recovery, tray recovery, tests, and operator docs. | Keep the disposable sentinel smoke as a required CI gate and compare global PF rules and enabled state before/after. |
