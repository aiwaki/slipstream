...
|---|---|---|---|---|
| 2026-07-30 | Protected Chromium top-frame correlation | Narrow browser observer and harness correction under review; no artifact or workstation mutation | Exact-main protected run `30561342773` passed packaged resources, the daemon-free boundary, account-backed owned-Geph initial/trayless/recovered Steam payload, unchanged system network state, and cleanup. The regional-denial browser scenario completed. The incomplete-response scenario received its first root response but produced no signal, reload, or subresource request. The correlation rejected an otherwise exact request when optional `parentFrameId` was omitted; accepting omission preserves every other scope check. Ordinary real-Chrome run `30563918442` reproduced the missing signal, and diagnostic run `30564634280` produced an empty owner-private trace: the command-line fixture navigation raced the fresh Manifest V3 worker before any observer was registered. Fixed-delay warm-up run `30565374098` also produced no worker trace; acknowledged warm-up run `30566477975` stayed on the extension page with zero fixture requests. The disposable harness now records bounded page/native readiness separately from worker readiness, retries the worker handshake, and never navigates before acknowledgement; no URL is traced and no production extension, routing, PF, DNS, Geph, installer, or system-network behavior changes. | Use the next real-browser trace to isolate extension loading, native-host registration, or worker startup. Then update the protected full-path harness consistently before its next exact-main qualification. Install only the artifact published after both browser scenarios and cleanup pass. |
