...
| 2026-07-15 | First-payload address race in the transparent handler | Implemented behind a policy-preserving wrapper | `spike/connection_probe.py` adapts the existing per-IP dialers to the owned connection-race I/O layer. Policy, route class, strategy, and backend are fixed before a race begins; a candidate succeeds only after the existing dialer receives first server bytes. The runtime handler now races resolved local addresses per strategy, app-owned Xbox DNS answers, and freshly proven Smart DNS answers. It does not race or reorder strategies, enter the Geph branch, mutate system DNS, or persist a circuit across requests. Fake handler tests prove that a stalled first Discord or Smart DNS edge is cancelled when a second edge returns payload, while Geph is forbidden. | Keep contract v1 frozen. Qualify this exact packaged daemon on disposable CI; add persistent circuit state or daemon IPv6 dialers only in separate evidence-backed changes. |
| 2026-07-15 | Packaged lifecycle Chrome process-group `EPERM` | Fixed and packaged-qualified | An earlier PR #107 run and the first two packaged runs for PR #109 ended with a bare `[Errno 1] Operation not permitted` after the private anchor became active, while adjacent `main` runs were green. Stage instrumentation localized post-merge occurrences to clean-profile Chrome probes, and operation instrumentation then reproduced the denial specifically at the root harness's `os.killpg(chrome_pgid, SIGTERM)` after the expected DOM had already loaded. Spawn, profile ownership, the daemon, and PF were not the failing operations. Chrome is launched in a dedicated session under the original runner UID/GID/groups, so the harness now delegates TERM/KILL for that already verified PGID to an isolated short-lived helper under the same identity. A missing group remains a successful stop; permission denial and every other helper failure remain fatal. No retry or `EPERM` suppression was added. The exact PR revision passed the complete packaged lifecycle, including all four fresh Chrome and Safari probes, tray crash/restart, sentinel preservation, and uninstall. | Preserve exact PID/PGID and browser identity checks; never replace this with broad process matching or alter routing/PF in response to a browser-cleanup denial. |
...
| 2026-07-11 | Live private anchor misread as legacy global PF | Fixed; disposable restart passed | A forced restart can leave the owned child anchor populated for the next daemon. Never run the one-time legacy global-rules migration while `com.apple/slipstream` already contains the expected redirect; otherwise an effective root listing can trigger `pfctl -f /etc/pf.conf` and discard unrelated PF state. | Keep the long-lived sentinel-state restart check; remove the legacy reload path after the migration window closes. |
