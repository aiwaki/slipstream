...
|---|---|---|---|---|
| 2026-07-27 | Chrome for Testing GUI bootstrap and audit session | GitHub-hosted sandbox boundary confirmed; explicit no-sandbox functional gate in progress | Protected runs `30270341870`, `30279476090`, and `30282319977` separately rejected root/manual credential drop, an exact `gui/<uid>` Aqua LaunchAgent, and the same LaunchAgent with `SessionCreate=true`. In every composition Chrome's sandboxed network service logged Mach rendezvous `Permission denied (1100)` while exact ownership and cleanup remained intact. Chromium source confirms that the macOS network-service sandbox is always enabled and requires an allowed `MachPortRendezvousServer` lookup; unlike other platforms it is not controlled by the `NetworkServiceSandbox` feature. The hosted gate therefore uses an explicit `--no-sandbox` switch only behind the existing disposable GitHub Actions guard, while the harness defaults to sandboxed Chrome elsewhere. A pass qualifies extension/native-host/route/resource composition but not production sandbox behavior, which remains a real-Aqua/self-hosted gate. Exact-label and process-group cleanup, PF/DNS/proxy/PAC/VPN non-mutation, and secret isolation remain unchanged. Codebase-memory MCP again returned `Transport closed`, so discovery used the documented narrow `rtk` fallback. | Pass ordinary PR and exact-main gates, then require learned exact-host routing, one reload, complete CSS/JavaScript/image delivery, DOM marker, browser-ready callback, and cleanup in protected CI. Preserve a separate production sandbox qualification gate. |
