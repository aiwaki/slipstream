...
|---|---|---|---|---|
| 2026-07-12 | OpenAI billing health canary | Fixed in this PR | `billing.openai.com` refused TLS both via the existing system Xbox DNS path and the owned Geph listener, while `chatgpt.com` produced a normal payload through that same Geph process. OpenAI's current support flow directs ChatGPT payment management through `chatgpt.com` and API billing through `platform.openai.com`; the billing hostname remains in geo-exit policy for real traffic, but it is not a reliable primary transport-health canary. Mark its canary warning-only so it stays visible in private diagnostics without turning a healthy OpenAI core route into tray-level attention. | Preserve the warning-only regression; investigate an owned-exit renewal only if core OpenAI probes also fail. |
