...
| 2026-07-15 | Packaged lifecycle Chrome process-group `EPERM` | Same-user signal fix under qualification | An earlier PR #107 run and the first two packaged runs for PR #109 ended with a bare `[Errno 1] Operation not permitted` after the private anchor became active, while adjacent `main` runs were green. Stage instrumentation localized post-merge occurrences to clean-profile Chrome probes, and operation instrumentation then reproduced the denial specifically at the root harness's `os.killpg(chrome_pgid, SIGTERM)` after the expected DOM had already loaded. Spawn, profile ownership, the daemon, and PF were not the failing operations. Chrome is launched in a dedicated session under the original runner UID/GID/groups, so the harness now delegates TERM/KILL for that already verified PGID to an isolated short-lived helper under the same identity. A missing group remains a successful stop; permission denial and every other helper failure remain fatal. No retry or `EPERM` suppression was added. | Require the packaged lifecycle gate on the exact revision. Preserve exact PID/PGID and browser identity checks; never replace this with broad process matching or alter routing/PF in response to a browser-cleanup denial. |
