...
|---|---|---|---|---|
| 2026-07-29 | Windows selected-stack/native-connector composition | Additive test-only composition implemented in PR #259; PR qualification pending | Keep all three predecessors frozen. Client payload must pass the existing byte-owner effect into the selected stack before the exact frame enters the existing bounded connector queue. Native backend reads use a separate bounded queue: an error must precede read progress, an accepted frame remains owned across selected-stack failure, and retry commits it once. Both directions revalidate flow, backend, and transport; the reverse path also retains exact sequence. Numeric loopback and the in-memory Layer 3 pair are the only effects. Discord/YouTube and UDP remain excluded from Geph, and no production host or external network state is touched. | Pass the focused/full Rust gates, project-wide checks, review, PR CI/audit, and exact-main gates. Then qualify packet flow on disposable Windows AMD64 and ARM64 before any production composition. |
