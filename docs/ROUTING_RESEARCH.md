...
|---|---|---|---|---|
| 2026-07-27 | Chrome for Testing GUI bootstrap and LaunchServices | LaunchServices composition merged; extensionless runner bundle compatibility isolated | PR #237 and exact-main CI `30295085145` passed with sandboxed LaunchServices launch and exact cleanup. Protected run `30295751995` passed real account-backed owned-Geph initial, trayless, and recovered payload (`HTTP/1.1 200 OK`, 68103 bytes), external-listener preservation, final cleanup, and unchanged system state. The browser failed before launch because `setup-chrome` exposed a metadata-valid bundle as the extensionless root `arm64`; passing it to `/usr/bin/open -a` produced `kLSNoExecutableErr (-10827)`. The narrow follow-up creates an owner-private `.app` symlink inside the disposable profile only for LaunchServices while keeping process ownership on the resolved source executable and removing the alias with the verified-clean profile. Codebase-memory MCP again returned `Transport closed`, so discovery used the documented narrow `rtk` fallback. | Pass ordinary PR and exact-main gates, then rerun protected CI and require sandboxed learned exact-host routing, one reload, complete CSS/JavaScript/image delivery, DOM marker, browser-ready callback, and exact cleanup. |
