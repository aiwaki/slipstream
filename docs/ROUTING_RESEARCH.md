...
|---|---|---|---|---|
| 2026-07-27 | Chrome for Testing GUI bootstrap and audit session | Root/manual-drop compositions rejected by protected runs `30265636270` and `30270341870`; temporary GUI LaunchAgent follow-up in progress | Run `30270341870` again passed real owned-Geph payload before and after recovery, trayless continuity, external-listener preservation, and fail-closed cleanup, but Chrome's sandboxed network service still logged Mach rendezvous `Permission denied (1100)`. This proves that `launchctl asuser` plus a later canonical groups/GID/UID drop is not a complete Aqua login/audit context. The follow-up writes an owner-private temporary plist and asks the console user's `gui/<uid>` launchd domain to create pinned Chrome for Testing as an `Aqua`/`Interactive` job. Bootstrap itself is inside the exact-target cleanup boundary. After review exposed early-exit gaps, teardown now retries only that exact target and retains the profile/plist unless both launchd and process-group absence are proven; a bootstrap that never yields a verified group cannot use launchd absence as a substitute. It changes no route, extension detector, PF/DNS/proxy/PAC/VPN state, or production distribution assumption. Codebase-memory MCP again returned `Transport closed`, so discovery used the documented narrow `rtk` fallback. | Pass ordinary PR and exact-main gates, then require the same learned exact-host route, one reload, complete CSS/JavaScript/image delivery, DOM marker, browser-ready callback, and cleanup in a new protected exact-main run. |
