...
|---|---|---|---|---|
| 2026-07-13 | Packaged tray-crash independence | Disposable CI implemented | Run the real packaged tray as the original user, kill only an exact UID/path-owned PID, and prove that fresh non-root HTTPS clients plus the same daemon/PF lifecycle survive tray crash and restart. Do not forward proxy or CI-secret environment into either child process. | Repeat with Safari/Chrome, physical route/lid transitions, and account-backed owned Geph on a disposable Mac. |
