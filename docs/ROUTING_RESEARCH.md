...
|---|---|---|---|---|
| 2026-07-26 | Generic browser page can load its document while mandatory same-origin assets stall after one TLS record | Fix implemented and locally qualified; exact artifact/browser gate pending | The root daemon remained continuously active before, during, and after the reported Chrome white page and Safari failure, so log inspection did not create the outage. The main document was insufficient evidence: `crystalidea.com/assets/css/all.css` repeatedly stopped at 16,237-16,238 bytes through the installed path, while a direct TLS/HTTP request received exactly one 16,384-byte TLS record and then made no progress. The system-selected address, app-owned Xbox DNS, public DNS, and all six local strategies used the same `173.230.144.164` edge and failed in that partial-stream shape. The explicitly enabled and ownership-verified bundled Geph returned HTTP 200 and all 33,140 CSS bytes in about 1.68 seconds. The new recovery therefore detects only a narrow one-record idle shape, exhausts system/Xbox plus distinct local strategies, checks the network-wide guard, proves real payload through owned Geph, and then learns a private, expiring exact-host runtime overlay. Static policy remains authoritative; Discord, YouTube, direct routes, external Geph, and external DNS/proxy/PAC/VPN cannot enter or be changed by this path. | Pass full CI, merge, install the exact artifact, and require complete CSS/JS plus nonblank DOM in fresh Safari and Chrome sessions before calling the incident fixed. |
