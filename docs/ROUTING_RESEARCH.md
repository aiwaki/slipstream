...
|---|---|---|---|---|
| 2026-07-18 | Native Windows TCP capture mechanism | WFP wire v1 implemented; native effects not composed | Use a minimal `ALE_CONNECT_REDIRECT` V4/V6 callout and the exact owned Rust service. A dynamic non-persistent `FWPM_CALLOUT` management object and its filters are one atomic session transaction after kernel callout registration and listener readiness; session close removes filters before kernel unregister. The fixed 128-byte v1 context and one-shot opaque redirect-record handoff now prove exact identity, bounds, stale rejection, and records-before-connect ordering without WFP or socket calls. WinDivert would require a packet NAT/TCP reconstruction path and does not fit this stream boundary. | Freeze the effect-injected runtime lifecycle reducer next. Keep direct connector v1 frozen and production no-network. |
