...
|---|---|---|---|---|
| 2026-07-29 | Disposable Wintun-to-selected-stack IPv4 UDP handoff | Local implementation and portable contracts pass; native evidence pending; no workstation or production-host mutation | The existing real Windows UDP socket and exact owned Wintun route now retain the exact captured Layer 3 packet and pass it through a reusable bounded `smoltcp 0.13.1` boundary. Only the stack-emitted response packet is injected back into Wintun, so successful receipt by the real OS socket proves checksums and the native return path. Fixed MTU, queues, poll steps, one request/response, deterministic input, and structured errors keep the handoff fail-closed. The stack remains a dev-only qualification dependency; source and contracts reject production service-host composition, default-route/DNS/external proxy/PAC/VPN mutation, external route ownership, and every Discord/YouTube Geph edge. | Run the exact native fixture on Windows AMD64 and ARM64 in PR CI. If both pass with cleanup, merge and record exact-main evidence; keep IPv6 UDP, TCP, and production-host composition as independent later gates. |
