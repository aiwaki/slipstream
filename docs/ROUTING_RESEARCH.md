...
| 2026-07-15 | Packaged lifecycle CI `EPERM` without stage context | Instrumented; repeated qualification passed | An earlier PR #107 run and the first two packaged runs for PR #109 ended with a bare `[Errno 1] Operation not permitted` after the private anchor became active, while adjacent `main` runs were green. The harness preserved daemon/PF log context but not its active operation, so the failures could not safely be assigned to install, tray, Chrome, Safari, signaling, or cleanup. Lifecycle failures now include the exact stage, sub-probe, and exception type. The instrumented PR #109 SHA then passed the complete packaged lifecycle twice, including tray crash/restart, fresh HTTPS/Chrome/Safari clients, wake/network re-arm, sentinel preservation, and uninstall. This establishes an intermittent harness/runner edge, not a routing or PF diagnosis. | If it recurs, preserve the named failing stage and change only that owned operation. Do not alter routing, permissions, or PF behavior in response to an unscoped `EPERM`. |
