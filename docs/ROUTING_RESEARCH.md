...
|---|---|---|---|---|
| 2026-07-26 | Packaged uninstall after qualification plist rewrite | Concrete lifecycle race fixed locally; requalification pending | PR #220 passed routing, Windows, dependency, and vendored-Geph jobs, but packaged lifecycle failed at uninstall with `cleanup incomplete: launchd job remains loaded`. The job had been booted out, its plist atomically patched with qualification-only environment and arguments, then bootstrapped again. Production cleanup retried only plist-path `launchctl bootout`, so the exact loaded KeepAlive service could remain present. Cleanup now addresses `system/dev.slipstream.tproxy` directly, falls back to the plist path for compatibility, and polls for exact job absence before any verified PID can be signalled. Local uninstall regressions and the complete Python/Rust suites pass. | Re-run PR #220 packaged lifecycle. Merge and install only after that disposable gate proves install, restart, browser sentinels, and complete uninstall. |
| 2026-07-26 | Unknown-host zero-payload routing and browser completeness | Same-request TCP recovery implemented locally; full gate pending | Crystalidea's later bare-HTML screenshot was stale browser cache after fresh Chrome and Safari rendered the complete page, so it is not current route evidence. Modrinth provides separate generic evidence: the system-selected TCP route closes during TLS even with HTTP/1.1; system, app-owned Xbox DNS, and public DNS return the same address set; the currently ownership-verified bundled Geph returns HTTP/2 200. Fresh Chrome additionally reports `ERR_QUIC_PROTOCOL_ERROR` on its normal path, while disabling QUIC exposes the same TCP close. The correction is not a Modrinth rule. For any eligible unknown TLS host, the current branch can retry the still-buffered first flight in the same request only while no server byte has reached the client, following `exact system -> per-connection app DNS -> local strategies -> verified owned Geph`. Geph requires confirmed closes from system and app-owned Xbox DNS, confirmed closes from every started address candidate in at least two distinct local strategies, a clear network-wide guard, and a real owned-Geph payload. Per-candidate payload, close, timeout/cancellation, and generic failure are distinct; only a confirmed close contributes evidence. Protected routes and external network owners remain excluded. | Pass full tests and CI, install the exact merge artifact, then validate with fresh browser profiles. Browser qualification must reject error pages and require the main document, mandatory CSS/JS, and a nonblank DOM. Design scoped QUIC handling separately; never add a global UDP/443 block. |
| 2026-07-26 | Generic browser page can load its document while mandatory same-origin assets truncate inside a later TLS record | Merged in PR #219; exact artifact/browser gate still pending | The root daemon remained continuously active before, during, and after the reported Chrome white page and Safari failure, so log inspection did not create the outage. The main document was insufficient evidence: `crystalidea.com/assets/css/all.css` repeatedly stopped at 16,237-16,238 bytes through the installed path. A direct traced TLS/HTTP request completed one application-data record, then received the public header of a second record declaring `16401` ciphertext bytes, but that record never completed and no more plaintext arrived. The system-selected address, app-owned Xbox DNS, public DNS, and all six local strategies used the same `173.230.144.164` edge and failed in that shape. The explicitly enabled and ownership-verified bundled Geph returned HTTP 200 and all 33,140 CSS bytes in about 1.68 seconds. Review rejected the first byte-range watchdog because a valid complete quiet response could share that size, rejected treating attempted system/Xbox stages as truncation proof, and found that a retained overlay could otherwise drift to an explicitly configured external listener. Later reviews found that dropping the overlay during owned-Geph cooldown would replay the already-failed local ladder instead of preserving the exact system fallback, and that confirmation cooldown state also needed an explicit cap. Recovery now incrementally parses bounded TLSCiphertext framing, requires separate incomplete-record proof from system, Xbox, and two local strategies, retains the network-wide guard and exact owned-Geph payload proof, caps and expires all confirmation state, and keeps the one-hour classification during cooldown while permitting a learned-route Geph dial only through the currently verified owned listener. Static policy remains authoritative; Discord, YouTube, direct routes, external Geph, and external DNS/proxy/PAC/VPN cannot enter or be changed by this path. | Install the exact merged artifact and require complete CSS/JS plus nonblank DOM in fresh Safari and Chrome sessions before calling the incident fixed. |
