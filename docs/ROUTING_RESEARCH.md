...
agree. Userspace byte-owner v1 now bridges actual payload through an injected
atomic effect: it retains exact bytes only when the supplied packet-flow
predecessor equals its last snapshot and the accepted queue grows by the exact
declared length, preserves
queued client payload until backend readiness, prevents replay of a committed
...
packet-flow high watermark. Both paths compare the exact predecessor and reject
same-timestamp stale evidence. The selected stack is still not
