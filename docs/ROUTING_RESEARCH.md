...
|---|---|---|---|---|
| 2026-07-29 | Disposable Windows packet-flow architecture gate | PR #261 head qualified on native AMD64/ARM64; exact-main pending; no workstation or production-host mutation | PR #261 head `0367addba3048bca58721577d35815607f33ca99` passed native run `30464226409`, CI `30464226625`, and audit `30464226363`. Each native runner executed the full frozen packet-flow contract and full stack/connector suite before the existing real Wintun lifecycle, exact-route, capture/injection, independent-route-owner, and cleanup gates. Review found that five transitive composition contracts were absent from the workflow path filters; the head now tracks every contract consumed by either invoked suite in both the pull-request and main-push path lists, and the regression test requires both occurrences. This proves architecture and native-loopback behavior adjacent to real Wintun effects, not an integrated Wintun-to-stack path. | Merge only after the evidence-only follow-up remains green, then require the same AMD64/ARM64 workflow on the exact main SHA. Keep Wintun-to-stack handoff and production SCM composition as later independent gates. |
