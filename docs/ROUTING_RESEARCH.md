...
|---|---|---|---|---|
| 2026-08-22 | Obscura assessment and scarce-token diagnostic lane | PR #362 implements a working non-release lane; external engine not adopted | Obscura is a separate Rust/V8 rendering engine, not Safari or Chrome, and its own documentation says stealth does not solve interactive CAPTCHA/challenge or IP rate limiting. Its result cannot represent the exact Slipstream PF/owned-Geph/Safari-or-Chrome path, so do not bundle or route through it. The owned one-host diagnostic instead uses the existing privacy-bounded classifier without starting the product, PF, Geph, root, or an account-backed flow. No-proxy local controls received HTTP 200 and roughly 14–15 KiB from XPersonatoy/Aikido before the document stalled; `Page.navigate` then timed out without a CDP `errorText`. Because transparent system routing is not asserted absent, the report correctly labels this an `unverified_local_environment`. The corrected tool continues bounded semantic observation and distinguishes an uncommitted `navigation_pending` state from an exact-host `readiness_visibility_missing` state; both have occurred for XPersonatoy under timing variation, while Aikido produced the latter, rather than falsely calling these `navigation_rejected`. An early local probe accepted consumer Google Chrome with a temporary `HOME` and caused a “Keychain Not Found” prompt; the corrected boundary rejects consumer Chrome before launch, accepts only Chrome for Testing or `chrome-headless-shell`, uses a mock Keychain, and stops the complete group before profile removal. Cleanup retains the unreaped leader while verifying that every current PGID member has the same UID, atomically signals that group, then reaps and only verifies absence; this avoids helper leaks and child-PID/PGID-reuse signalling. A completed finding requires a valid semantic or privacy-reduced target-state observation and is distinct from target usability, while harness/report/cleanup failure remains nonzero. The manual read-only CI workflow may run one host without unrelated release jobs, but its result is explicitly non-release evidence. | Finish PR #362 review and CI without merging it into the still-active `f16bb7c` candidate. Preserve the full protected release matrix. A future account-backed one-host workflow would still consume a broker token and cannot evade the daily limit. |
| 2026-08-22 | Live-site release-readiness false positive and masked Weather state | Fixed in merged PR #361; routing/product code unchanged | Protected readiness `32572723728` proved the earlier Safari/Chrome lifecycle correction: Aikido and Capacitor were usable in both browsers, while XPersonatoy and Weather reached their exact host deadlines in both browsers. In one bounded direct XPersonatoy observation, the generic `captcha` marker that made the detector non-passing occurred only in a dormant script on an otherwise StarrToy-shaped document. The harness had searched raw Safari source and raw Chrome DOM alike, so generic marker presence alone was not a visible challenge. PR #361 retains strong raw challenge markers but counts generic `captcha` only from one page-side visible-text-or-widget boolean, and keeps Weather's pending document terminal while reporting `readiness_document_pending_semantic_ready` only after its fixed semantic conditions pass. No route, deadline, control, host, or browser rule changes. | Candidate `f16bb7c9a58065c749cb13b24c42c3d6d0389a05` has exact-main CI `32575593760`, dependency audit `32575593677`, and artifact `9476551838`; do not retry unchanged `111000c`. One next-day protected owned-Geph qualification and one bound readiness remain the release gates. |
| 2026-08-20 | Reviewed `geph5-client` 0.3.9 transition | Source, dependency, arm64 build, and compatibility review green; universal and live gates open | PR #280 pins crates.io SHA-256 `6d1d641b2548eab0b4e0e73b6bff4961a1faaab1c333c23d4795e6960cdf11eb` and reviewed lock SHA-256 `31458e3754c4353c43aabd6813e77a523fd10451942f930811260eff9101bd3a`. The dual-target 484-package graph has zero blocking advisories and resolves the fixable branch to `h2 0.4.17`; residual `h2 0.3.27` remains an exact reviewed exception. CLI, generated YAML fields, AWS transport, loopback SOCKS bind, authentication, and control protocol remain compatible. The useful runtime change preserves responses after a client half-close. It does not make an open SOCKS listener payload-ready, and SOCKS IPv6 literals remain unsupported; exact hostnames continue to be required for that path. | Prefer 0.3.9-r1 over the lower-delta 0.3.0-r2 only after universal build, generated-config startup, stable successor plus payload canary, owned-Geph restart, and protected live routing gates pass. Keep 0.3.0-r2 as fallback if those gates regress. |
| 2026-08-20 | Geph `priority_race` keys are millisecond start delays | Reviewed 0.3.0 and 0.3.9 sources are byte-identical in this behavior | `priority_race.rs` converts each map key with `Duration::from_millis`, sleeps that delay, then enters `race_ok`. Slipstream therefore starts the unpinned fronted source at 0 ms, the pinned fronted source at 300 ms, and AWS Lambda at 1500 ms. The old Rust comment incorrectly called AWS the high-first primary; runtime ordering was already the delay-based first-success race. | Correct the comment without changing the reviewed broker configuration during the 0.3.9 transition. Treat future key edits as start-delay changes and requalify owned-Geph payload and latency. |
| 2026-08-13 | Deterministic macOS IPv6 PF/NATLOOK fixture | Local full Python regression green; disposable kernel proof pending | Two seemingly plausible targets did not prove the production-family redirect: a static scoped link-local address depended on a neighbor, while an address assigned to the Mac itself remained a scoped local identity and did not traverse the inbound `rdr on lo0` path. The gate now transactionally owns one fixed RFC3849 `/128` alias on `lo0`. Before mutation it requires the address absent globally, rejects an existing `lo0` route, and records the exact `lo0` IPv6 set; after the probe it removes only that alias and requires the address absent, route snapshot restored, and IPv6 set byte-for-byte equivalent after canonicalization. Partial add, collision, verification exception, cleanup failure, and probe exception all fail closed. This live packet proof covers `inet6` loopback `rdr` plus DIOCNATLOOK; the non-`lo0` IPv6 `route-to` branch remains an exact loaded-rule/static assertion. | Run the high-port smoke on a disposable macOS runner. Do not claim external IPv6 `route-to` packet proof from the alias fixture or weaken the required cleanup evidence. |
...
| 2026-08-12 | Protected extensionless automatic browser path | Qualified on exact main; artifact uninstalled | PR #335 merged the immutable-attestation sequencing correction as `fcbcedaa4e812e5080a00ce3d7f4bb8e89c6aff5` after exact-head review, six green PR checks, and green exact-main CI `31587057669`, audit `31587057685`, and native Windows run `31587057755`. Protected run `31587594114` passed `original -> worker -> original`; worker start was 17,087 ms, exact original retry 28,899 ms, and styled completion 25,713 ms with one CSS, JavaScript, image, and ready callback, extensions disabled, no manual reload, and clean worker cleanup. All three unpacked semantic scenarios also completed their exact reload/resource contracts, including one pending-navigation v3 signal. Owned Geph returned 67,973-byte Steam HTTP 200 payloads initially, trayless, and recovered; its external listener was preserved. Final root daemon, user state, keychain, runtime, sockets, manifests, and private PF anchor were absent, and system network state was not mutated. Artifact `9137909918` has GitHub digest `sha256:9348c6a2d36e718edc4bc905bbb94b622b7c547d7605decebee6136ede7d00df`; the contained release ZIP SHA-256 is `1fbd3ff7da2143396cc45292217c5692c18783910c4897308d9e4f767103a9ab`, and both its recorded checksum and `unzip -t` pass. | Merge evidence-only PR #336 with exact-head review and all six checks. Do not repeat this protected run or install during the docs transaction. A later controlled workstation transaction may use only artifact `9137909918` and must preserve every external owner plus roll back on the first failed required smoke. |
| 2026-08-12 | Protected automatic-browser restart crossed the immutable install-attestation PID boundary | Failed closed on exact main; qualification-only correction in PR #335 | PR #334 merged as `a49e0fc82018569daf7cbccff0972d8408415bb7` after exact-head review, six green PR checks, and green exact-main CI `31584859058`, audit `31584858959`, and native Windows run `31584859039`. Its sole protected run `31585408666` passed the exact build/signature checks, daemon-free boundary, account-backed owned-Geph lifecycle and payload, external-listener preservation, and always-run system cleanup. The combined qualification then failed before browser launch with `launchd attestation mismatch`: the installer had correctly attested initial PID `27124`, while the harness deliberately replaced that daemon to inject the frozen disposable browser environment and then asked the immutable install record to equal the new runtime PID. No product defect, artifact, workstation change, or external-owner mutation was accepted. The correction validates the full install payload and attestation before the intentional restart, then verifies the fresh replacement PID against the exact installed command after bootstrap. It neither rewrites attestation nor changes product code. The codebase graph was retried first and returned `Transport closed`; bounded source inspection used the documented fallback. Local verification passes `1198` tests plus `54` subtests, focused 66-test verification, compilation, and diff checks. | Review and merge PR #335 with all six checks, require fresh exact-main CI/audit/native Windows, then dispatch one new protected run for that exact SHA. Require the automatic browser evidence and complete cleanup before any artifact gains installation authority. |
| 2026-08-12 | Protected exact-artifact automatic browser qualification | Implemented locally; no protected artifact or workstation mutation | PR #333 review identified that the protected workflow's existing regional-denial, incomplete-response, and pending-navigation scenarios all depend on an unpacked extension, so they cannot prove the production extensionless worker included in the exact app. The protected harness now composes its closed HTTPS fixture before install, patches only the installed disposable LaunchDaemon's frozen browser override environment, restarts that exact daemon, and runs the existing full `original -> worker -> original` navigation assertion before continuing the semantic-extension matrix. The assertion requires complete CSS, JavaScript, image, and ready callbacks, extensions disabled, no manual reload, and exact worker/profile cleanup. Account-backed owned Geph remains live and is rechecked after restart; normal uninstall and final workflow checks still require all product/user state absent and external PF state unchanged. The workflow pins the same complete LaunchServices-valid Chrome for Testing `151.0.7922.77` as ordinary packaged CI. Production code, routing policy, browser idle behavior, and external DNS/proxy/PAC/VPN/PF owners are unchanged. Local verification passes `1197` tests plus `54` subtests. | Review and merge only with all six checks, pass fresh exact-main CI/audit/native Windows, then dispatch one protected run for that exact main SHA and inspect the automatic-path plus cleanup evidence. Do not install any existing or branch artifact. |
| 2026-08-12 | Packaged pending-navigation result raced Chrome cleanup and worker-relay recursion | Qualified on exact main in PR #332 | Documentation-only head `79abf93cfa61a448ed7681567f8f3c226b781cd1` left product code unchanged but packaged job `94021442646` failed after roots `original -> worker`: the worker logged `browser_worker_failed:submit_rejected`, then the daemon retried stale secondary jobs as `claimed_job_invalid`. The previous exact code had passed packaged job `94019999739`, making this a real intermittent timing boundary. Moving submission before cleanup was necessary but packaged job `94032209506` reproduced the same rejection, proving that a rejected result could still lose its guard too early. An accepted result now keeps its exact-host guard until the one-shot launcher positively confirms exact worker and Chrome/profile cleanup, including cleanup beyond the original capability expiry. Each launcher-generated random 64-bit ID crosses only the owner-only local claim/submit IPC and binds guards to the exact capability/launch pair. Launcher bootstrap/start/wait/disappearance/PID/identity failures, worker-reported Chrome/profile cleanup failure, and an unknown/crashed exit retain that pair even after successful `bootout`, while a later confirmed launch can release only its own pairs; qualification logs never print the ID. The immediately preceding release's otherwise exact stale plist remains cleanable only in its precise legacy environment shape without the new key. The broker reports the exact claim before delivery, so expiry, rejection, revocation, or relay teardown of that active claimed capability promotes its host to the same cleanup-bound guard; unclaimed equivalents remain bounded to original expiry. Rejection logging is limited to a static class. A rejected enqueue is removed with no guard. After a false notification, queue discard and the no-worker proof share the lifecycle lock, so a claimed or restarting worker prevents discard; an ambiguous notification exception remains guarded. Capabilities and guards share one 32-entry admission bound; a new job is rejected at capacity rather than evicting live authority or an unexpired guard. Head `349e00e` passed all six PR checks including packaged job `94034409872`. Cleanup-bound head `1a53d37` then removed the original failure but packaged job `94038042816` recorded only `original`, with a primary `ci_origin_invalid`, one start timeout, and three expired-lease `claimed_job_invalid` retries. The closed CI origin had received an unrelated background-host job from the runner-wide transparent path. Under the exact three disposable markers, capability admission now requires the same exact host, public IP, and port-443 tuple already authorized for fixture loopback mapping; malformed configuration fails closed and production remains unchanged. Head `3a87c02` passed all six checks, and packaged jobs `94042532588` plus explicit repetition `94044217785` each passed the original-to-worker-to-original lifecycle and exact cleanup. Cleanup-confirmation head `0512a0bb` also passed all six checks including packaged job `94047866907`; exact-launch head `2d977cf6` passed all six checks including packaged job `94053777620`. Fresh review proved that allowed worker execution can cross capability TTL, that neither successful LaunchAgent cleanup nor later-launch cleanup proves earlier worker/browser teardown, that worker-owned Chrome cleanup must also be proven, and that upgrades must retain exact legacy stale-worker cleanup. Final reviewed head `0d30a371` passed all six PR checks; packaged jobs `94058618509` and `94060386147` independently proved the complete lifecycle. It merged as `b5514a693cfa412650d54878c0ccee8d8b664a07`; exact-main CI `31580485874`, audit `31580485872`, and native Windows run `31580485841` passed. Exact-main packaged job `94062155231` again proved `original -> worker -> original`, styled readiness, exact cleanup/uninstall, preserved sentinels, and unchanged global PF. | Keep the exact-main gates required. Before any protected run or workstation transaction, add the extensionless automatic worker path to the protected exact-artifact gate; its current unpacked-extension scenarios cannot supply that proof. Do not install an existing, branch, or merely protected artifact. |
