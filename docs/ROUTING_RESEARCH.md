...
|---|---|---|---|---|
| 2026-07-14 | Address racing and scoped circuit breaking | Pure cross-language contracts implemented | A resolver result is an ordered set of expiring candidates, not one address. The planner interleaves the preferred and alternate families, starts at most one candidate per reduction, bounds concurrent attempts under one deadline, and deterministically cancels losers. Repeated backend failure opens only the circuit keyed by service group, route class, and backend; half-open probes use a bounded permit and late completions cannot revive an open circuit. Neither reducer performs I/O or changes routing policy, PF, DNS, proxy, PAC, or VPN state. | Integrate behind fake DNS/socket adapters first. Add runtime outcome evidence only after the same vectors pass every platform core; never make a circuit opening imply Geph fallback. |
