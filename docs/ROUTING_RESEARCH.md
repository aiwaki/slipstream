...
| 2026-07-12 | Partial local stream stall after clean client EOF | Implemented; packaged validation pending | A local TLS route can return an initial payload above the former 8 KiB success threshold and then stop. On the affected network, `crystalidea.com` returned HTTP 200 with 16,366 of 21,726 bytes before a 25 s client timeout. The clean-EOF reducer was correct but unreachable: `_handle_impl` used `asyncio.gather`, so after the client timed out it waited forever for a stalled server-to-client `splice`, never recording the exact-host outcome. The generic local relay now records which direction completed first and cancels the useless peer task; only two client-first EOFs after at least 15 s without downstream progress for the same unknown host in five minutes demote the exact strategy. The next retry makes an app-owned RFC 8484 Xbox DNS query and tries its answer locally with plain TLS. The current resolver and Xbox DNS both returned `173.230.144.164` for this host, so DNS is an exact local attempt, not a promise of an alternate edge. Server-first completion clears pending evidence. This path never selects or learns Geph. | Keep the relay-direction, repeated-EOF, and protected-host regression tests; validate state transition plus a packaged retry. |
