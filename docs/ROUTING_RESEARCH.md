...
|---|---|---|---|---|
| 2026-07-11 | Live private anchor misread as legacy global PF | Fixed; disposable restart recheck pending | A forced restart can leave the owned child anchor populated for the next daemon. Never run the one-time legacy global-rules migration while `com.apple/slipstream` already contains the expected redirect; otherwise an effective root listing can trigger `pfctl -f /etc/pf.conf` and discard unrelated PF state. | Keep the long-lived sentinel-state restart check; remove the legacy reload path after the migration window closes. |
| 2026-07-11 | Script-mode installed lifecycle | Implemented; disposable CI pending | The dev installer omitted `primes.py`, so a source-installed LaunchDaemon could crash-loop before reaching routing. Copy the complete local Python payload, remove status/TGWS runtime artifacts on uninstall, and qualify cold install, active restart, uninstall, sentinel rules, and a long-lived sentinel PF state on a disposable runner only. | Keep packaged-app install/update qualification separate; never run this destructive lifecycle smoke on a workstation. |
