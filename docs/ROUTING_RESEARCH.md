...
|---|---|---|---|---|
| 2026-07-28 | Protected owned-Geph/Chromium cleanup interleaving | Coordination fix merged and exact-main gates passed; protected requalification awaits upstream reset | PR #251 merged as main `8c6814fd6130d7b08e339d6d41eb37922a66973a`; exact-main CI `30396785782` and audit `30396784419` passed. The sole protected run `30397332251` built and verified the exact bundle, established the daemon-free boundary, then received the already-classified upstream `cannot get token: rate limited` failure after shared-secret authentication and tunnel establishment. Because the producer aborted after publishing `ready`, it began user-resource cleanup while the semantic consumer still owned the root daemon and Chromium native-host manifest. That produced secondary `root daemon boundary changed` and missing-manifest cleanup errors; the workflow's independent always-run cleanup still passed, and no artifact was uploaded. PR #252 retains a late abort, waits for the exact private `release`, then reports the original failure and cleans up in ownership order. Its exact-main CI `30399287525` and audit `30399287465` passed on `c9d3117fe2ab37946522ddcd16cddb64efe4b55d`. | Dispatch the protected workflow exactly once for the current main SHA only after the upstream UTC-day budget resets. Do not install until the complete gate publishes its exact artifact. |
| 2026-07-28 | Protected incomplete-response v2 runtime signal | Account-backed run failed safely; no artifact or workstation mutation | PR #250 merged as main `49a79d0332e96c067acdaeaf9983433f8fcaff7d`; exact-main CI `30392495460` and audit `30392494245` passed. The sole protected run `30393151116` proved real owned-Geph Steam payload before tray exit, trayless, and after KeepAlive recovery. Its browser phase timed out because `webNavigation.onErrorOccurred` did not report the post-commit truncated-body fixture. Exact cleanup passed and no artifact was published. Chromium now observes read-only `webRequest.onErrorOccurred` only for HTTPS top-level GET requests and retains the same two exact error allowlist; the protected harness also reports the scenario and bounded fixture counters on timeout. | Pass focused/full checks in a small PR, then exact-main CI/audit and one new main-only account-backed run. Do not install unless both browser scenarios and final cleanup publish the exact qualified artifact. |
