...
|---|---|---|---|---|
| 2026-08-22 | Obscura assessment and scarce-token diagnostic lane | External engine not adopted; Slipstream-owned diagnostic lane proposed | Obscura is a separate Rust/V8 rendering engine, not Safari or Chrome, and its own documentation says stealth does not solve interactive CAPTCHA/challenge or IP rate limiting. Its stealth fingerprint, TLS/root-store, tracker-blocking, and profile changes would make a response a different-client result, not evidence for the exact Slipstream PF/owned-Geph/Safari-or-Chrome path. It also adds an untrusted-JS/V8 supply chain and must not run beside the privileged daemon. Do not bundle or route through it. Add a one-host, direct headless-Chrome diagnostic that uses the existing privacy-bounded classifier but starts no product, PF, Geph, root, or account-backed flow; output is explicitly non-release evidence. A separate manual read-only CI workflow may run that one host without all release jobs. | Validate the owned diagnostic implementation and preserve the full protected release matrix. If a concrete account-backed one-host investigation is needed later, design a separately protected cleanup-bound workflow; it still consumes a broker token and cannot be used to evade the daily limit. |
