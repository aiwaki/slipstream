...
|---|---|---|---|---|
| 2026-07-29 | Disposable Windows packet-flow architecture gate | Implementation in progress; no workstation or production-host mutation | The existing Wintun workflow already runs independently on native AMD64 and ARM64 but did not track or execute the newer packet-flow, selected-stack, or connector-composition sources. Make those contracts and crates trigger the workflow. On each exact native runner, execute the full frozen packet-flow contract and full stack/connector suite before the existing real Wintun lifecycle, exact-route, capture/injection, independent-route-owner, and cleanup gates. Freeze those workflow paths and commands in the composition contract test. This proves architecture and native-loopback behavior adjacent to real Wintun effects, not an integrated Wintun-to-stack path. | Pass local tests and lint, then PR and exact-main AMD64/ARM64 workflow jobs. Keep Wintun-to-stack handoff and production SCM composition as later independent gates. |
