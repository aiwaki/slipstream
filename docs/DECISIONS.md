...
| 2026-07-11 | An abnormal client-side transport abort or failed downstream write after a local stream pause demotes only the exact generic strategy. The next retry may query Xbox DNS directly through its verified DoH endpoint for that exact unknown hostname and try the result locally with plain TLS. An orderly keep-alive EOF is not a failure signal. A generic low-content stall storm must take the same one-shot local DNS check before geo-exit proof. This changes no macOS DNS setting and never learns or routes the host through Geph. | Active |
