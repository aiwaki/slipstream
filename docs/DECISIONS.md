...
| 2026-07-12 | A generic local relay must settle when either direction completes. If the client ends first, cancel the undeliverable upstream read and record the client-first outcome before policy runs; if the server ends first, do not treat cancellation of the client pump as a clean EOF. This lifecycle rule applies only to the generic local-desync path and does not alter geo-exit or protected-service routing. | Active |
