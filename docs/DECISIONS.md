...
| 2026-07-12 | A secondary geo-exit canary may be warning-only when the service's primary current endpoint remains healthy. The endpoint stays in routing policy and its evidence remains available in private diagnostics; it must not turn the whole route class into tray-level attention by itself. | Active |
