...
4. Completed on the exact merged commit: terminating the exact child process
   ...
   first IPv4 UDP subgate. PR #200's candidate adds a separate real TCP-stream
   gate with a checksum-valid synthetic handshake, activation exchange,
   fail-closed exact-route rollback if the established stream reaches capture,
   retransmission recovery, and an unconditional same-stream post-removal
   exchange. Native and exact-main evidence remain required before that TCP
   gate closes. Bounded capture expiry/removal and explicit external-VPN
