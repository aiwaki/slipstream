...
3. Completed on the exact merged commit: the smallest native lifecycle on AMD64
   ...
   authorization.
4. Completed on the exact merged commit: terminating the exact child process
   ...
   passed that gate on native AMD64 and ARM64. PR #198's exact main commit
   passed the next disposable subgate on both architectures: two owned IPv4
   Wintun interfaces and one owned non-default `/24` baseline route prove a
   connected UDP socket works before exact `/32` activation, then require either
   ...
   first IPv4 UDP subgate. PR #200 adds a separate real
   ...
   closed. The next candidate uses a separately spawned exact child as an
   independent VPN-like network owner: it retains `198.18.0.2/32` and one
...
7. Qualify the remaining reboot, sleep/wake, update, uninstall, broader crash,
   ...
   `7f265549d727db93b8c0a7861808cc5f59784527`. The following bounded subgate
   applied that invariant to two byte-distinct copies of the real production
   service host through its public `manage install/uninstall` CLI. It proved
   ...
   subgate installed that same real host, revalidated its exact SCM PID,
   canonical image path, SHA-256, owner and active records, terminated only
   the retained verified process handle, and required public `manage recover`
   to replace the process instance and reset its crash budget. Replacement is
   proven by PID plus Windows creation time so PID reuse cannot create a false
   pass; repeated recovery is idempotent, an independent sentinel survives,
   and uninstall proves exact absence. Done in PR #277; exact-main native run
