...
| Strategy cache and policy | bounded/versioned cache plus explicit policy tables, diagnostic policy hash, signed-bundle validator, local persist, rollback, and disabled remote fetch/health-gate plumbing | scheduled signed remote fetch/apply |
...
- Move explicit host policy, attempt limits, and policy hash into a scheduled
  signed remote fetch/apply flow using the local rollback and health-gate path.
