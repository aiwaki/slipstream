...
| Strategy cache and policy | bounded/versioned cache plus explicit policy tables, diagnostic policy hash, signed-bundle validator, local persist, and rollback | signed remote fetch/apply |
...
- Move explicit host policy, attempt limits, and policy hash into a signed remote
  fetch/apply flow using the local rollback path.
