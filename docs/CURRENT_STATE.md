...
with the matching queue delta. Ordinary terminal cleanup removes only its exact
flow; explicit generation retirement is high-watermark bounded, and both paths
reject stale predecessor state even at the same timestamp. The next safe M4
