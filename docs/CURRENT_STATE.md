...
|---|---|---|
| M0 - Safe Base | Automatic rollback guard implemented locally; packaged qualification pending | Private-anchor lifecycle, owned PF tokens, exact process identity, protected secrets, and script/packaged lifecycle CI are implemented. The PR #172 artifact's broad HTTPS regression was reproduced in the disposable packaged gate. PR #174 fixed app-bundle removal, and PR #175 repaired exact-system HTTPS passthrough and bounded half-close handling without changing protected routing policy. The current branch adds pre/post-arm HTTPS qualification through the console user's unchanged system route, automatic private-anchor rollback, and listener-before-cleanup ordering when PF removal fails. Local Python, Rust, Clippy, and exact hidden-helper checks pass. This is not release evidence until review and the disposable packaged Safari/Chrome/PF-sentinel lifecycle pass on the exact branch and merge commit. Do not reinstall unattended. |
