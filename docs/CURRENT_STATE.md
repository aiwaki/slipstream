...
main-document probe: mandatory same-origin CSS stalls after one TLS record on
the system, Xbox DNS, and every local strategy, while verified owned Geph
delivers the complete resource. Branch
`codex/unknown-partial-stall-fallback` implements the bounded exact-host
evidence path; it has passed the full Python suite locally but is not merged,
installed, or browser-qualified yet.
