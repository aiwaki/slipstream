...
The active follow-up wraps only the qualification-browser command in
`launchctl asuser <exact-console-uid>` after retaining the same UID, GID,
supplementary groups, owner-only profile, and dedicated process group. It uses
no shell or `sudo`. Product routing, the frozen extension, static policy, and
external network ownership do not change. Completion still requires the
