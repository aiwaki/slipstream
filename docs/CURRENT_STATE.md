...
`bd3f06b94799b92cda6581fc83be1f18817a2126`. PR #234 moved Chrome for
Testing into an exact temporary LaunchAgent in `gui/<console-uid>`. Exact-main
CI run `30278757476` and audit run `30278757357` passed. Protected run
`30279476090` independently reached the account-backed runtime gate but again
recorded Chrome network-service Mach rendezvous lookup failures with
`Permission denied (1100)`. The evidence now rejects both root/manual
credential drop and an `Aqua`-limited user LaunchAgent without an explicit
security audit session.

The active follow-up keeps the exact user LaunchAgent but adds
`SessionCreate=true`, the launchd contract for spawning the job into a new
security audit session. The owner-private plist carries the already validated
...
workstation. The next action is to pass the audit-session follow-up through PR
and exact-main gates, rerun protected qualification on that exact merge commit,
and record the resulting run ID.
...
| M1 - Autonomous Routing V1 | Partial; protected packaged Chromium qualification is the active gate | PRs #219-#234 cover generic transport recovery, the strict semantic contract, owner-only daemon IPC, independent exact-host owned-Geph confirmation, exact Chromium-origin authentication, one bounded reload, unsigned Safari packaging, pinned Chrome for Testing, exact profile-local native-host composition, exact temporary LaunchAgent ownership, and fail-closed cleanup. Exact-main CI `30278757476` and audit `30278757357` passed on `bd3f06b`. Protected run `30279476090` proved that an exact `gui/<uid>` Aqua LaunchAgent without `SessionCreate` still leaves sandboxed Chrome helpers unable to resolve the browser's Mach rendezvous service. The active follow-up adds the explicit launchd security audit session while preserving exact-label plus process-group cleanup; it must still prove a learned route, one reload, CSS/JavaScript/image completion, DOM mutation, and one browser-ready callback. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. Reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
