...
`d9ccfc05b19fb75f1892eace743a5cb744c8b6ca`. PR #225 merged the first
browser adapter after all required PR and exact-main checks passed. Its generic
MV3 detector recognizes strong regional-denial language locally and sends only
category plus confidence. The service worker derives hostname from
browser-owned top-level sender metadata; an exact-origin Rust native host
revalidates the frozen contract and forwards it to the daemon's owner-only
socket. Registration is user-scoped, atomic, mode `0600`, and limited to a real
macOS app bundle; uninstall removes only an exact owned manifest. When daemon
confirmation is scheduled, the same tab may reload once after the six-second
confirmation budget, only if its hostname is unchanged. Static direct,
direct-first, local-bypass, and reviewed geo-exit policy remains authoritative;
semantic evidence never enters the transport-failure candidate map or selects
a route by itself.

The current `codex/safari-semantic-companion` branch adds an unsigned Safari
source and packaging path without changing installed routing. It reuses the
bounded detector and frozen signal but uses Apple's containing-app/app-extension
native-messaging boundary. The Swift bridge enforces the exact signal and
response shapes and talks to the same owner-only daemon socket. A fresh Xcode
project generated by Apple's packager compiles as a macOS 13 app plus `.appex`
without signing; the build script immediately unregisters only its generated
container app, including its embedded extension, from LaunchServices and never
opens or enables either one.
No extension or app has been installed. Chrome Web Store publication, real packaged Chrome
qualification, and a disposable signed Safari sandbox/socket qualification
remain open.
...
| M1 - Autonomous Routing V1 | Partial; generic transport, owned-Geph payload recovery, daemon semantic confirmation, and Chromium preview merged; Safari unsigned build in progress | PRs #219-#225 cover incomplete TLS records, replay-safe zero-payload continuation, bounded early server-first cuts, the control-plane/payload-health mismatch, the strict language-neutral semantic contract, owner-only session-aware daemon IPC, independent exact-host owned-Geph confirmation without site rules, exact Chromium-origin authentication, and one bounded same-host reload after confirmation scheduling. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. The current branch adds a separately packaged Safari app extension and strict Swift bridge without installation. Packaged real-browser qualification, reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
...
[CI run 30226683250](https://github.com/aiwaki/slipstream/actions/runs/30226683250).
The dependency and vendored-Geph audits passed in
[audit run 30226683251](https://github.com/aiwaki/slipstream/actions/runs/30226683251).
