...
Review and select a native Windows capture mechanism behind capture-source v1,
then qualify it on disposable Windows for exact ownership, bounded shutdown,
crash/reboot compensation, and fail-safe disable/uninstall. Keep the production
SCM host no-network until those gates pass. Do not add a resolver, local/geo
backends, or installer UI inside the capture/relay boundary.
