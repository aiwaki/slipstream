...
PR #194's exact head passed the equivalent isolated IPv6 selection subgate on
native AMD64 and ARM64 using host-order `IPV6_UNICAST_IF`, two exact `/128`
addresses, and one explicitly owned test-only baseline `/64`. Require a clean
exact-head review, merge, and exact-main rerun before advancing. Do not combine
it with packet delivery. Actual packet delivery, fresh
