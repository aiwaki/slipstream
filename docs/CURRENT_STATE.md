...
`a27e28a3d9683f755b107c8bb19019b28bc79850`. PR #233 added
`launchctl asuser` before a root-only helper dropped to the console user's POSIX
identity. Exact-main CI run `30269811927` and audit run `30269811765` passed.
Protected run `30270341870` independently passed the real owned-Geph payload
three times, trayless continuity, LaunchAgent recovery, fail-closed cleanup,
and external-listener preservation. Chrome's network service still repeatedly
failed its Mach rendezvous lookup with `Permission denied (1100)`. The result
rejects root launch plus manual credential drop: it does not create a complete
Aqua login/audit context for sandboxed Chrome children.

The active follow-up loads Chrome for Testing as an exact temporary LaunchAgent
...
workstation. The next action is to merge this temporary-LaunchAgent follow-up
after required CI, pass exact-main CI, rerun protected qualification on that
exact merge commit, and record the resulting run ID.
...
| M1 - Autonomous Routing V1 | Partial; protected packaged Chromium qualification is the active gate | PRs #219-#233 cover generic transport recovery, the strict semantic contract, owner-only daemon IPC, independent exact-host owned-Geph confirmation, exact Chromium-origin authentication, one bounded reload, unsigned Safari packaging, pinned Chrome for Testing, exact profile-local native-host composition, and fail-closed cleanup. Exact-main CI `30269811927` and audit `30269811765` passed on `a27e28a`. Protected run `30270341870` passed real owned-Geph payload, trayless continuity, recovery, cleanup, and external-listener preservation, but proved that `launchctl asuser` followed by manual credential drop still leaves sandboxed Chrome helpers unable to resolve the browser's Mach rendezvous service. The active follow-up launches Chrome as an exact temporary `gui/<uid>` Aqua LaunchAgent and retains exact-label plus process-group cleanup; it must still prove a learned route, one reload, CSS/JavaScript/image completion, DOM mutation, and one browser-ready callback. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. Reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
