...
Do not reinstall or re-arm Slipstream on the primary workstation. First repair
and independently qualify two regressions: direct and unknown HTTPS must retain
an exact-destination, byte-transparent baseline path instead of entering the
adaptive local engine on first contact, and tray uninstall must reliably remove
the validated app bundle after privileged cleanup. Add long-lived
WebSocket/streaming, no-SNI/ECH, external-DNS, external-VPN, and detached
self-removal regressions. Only then produce a new versioned preview artifact and
repeat the primary smoke with an explicit rollback guard.
