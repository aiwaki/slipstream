...
`e4eba7b7322d0a922f61b35bf31d2c427fd0c173`. PR #232 pinned Chrome for
Testing and copied the exact packaged native-host manifest into its disposable
profile. Exact-main CI run `30265221508` and audit run `30265221531` passed.
Protected run `30265636270` independently passed the real owned-Geph payload,
trayless continuity, recovery, and fail-closed cleanup gates, but Chrome's
network service repeatedly failed its Mach rendezvous bootstrap lookup with
`Permission denied (1100)` and the fixture received no request. The harness
dropped to the console user's UID but did not enter that user's GUI bootstrap
namespace. The result rejects that launch composition; it is not evidence
against Chrome for Testing, the packaged native host, the daemon semantic
contract, or owned Geph.

The active follow-up lets root enter the exact console user's namespace through
`launchctl asuser`, then uses a fixed disposable-CI helper to set the already
resolved supplementary groups, GID, and UID immediately before directly
executing Chrome. The owner-only profile and dedicated process group remain the
same, and no shell or `sudo` is involved. Product routing, the frozen extension,
static policy, and external network ownership do not change. Completion still requires the
independently confirmed future exact-host route, exactly one reload, every
...
GUI-bootstrap follow-up after required CI, pass exact-main CI, rerun the
protected qualification on that exact merge commit, and record the resulting
run ID.
...
| M1 - Autonomous Routing V1 | Partial; protected packaged Chromium qualification is the active gate | PRs #219-#232 cover generic transport recovery, the strict semantic contract, owner-only daemon IPC, independent exact-host owned-Geph confirmation, exact Chromium-origin authentication, one bounded reload, unsigned Safari packaging, pinned Chrome for Testing, exact profile-local native-host composition, and fail-closed cleanup. Exact-main CI `30265221508` passed on `e4eba7b`. Protected run `30265636270` passed real owned-Geph payload, trayless continuity, recovery, and cleanup, but Chrome's network service could not resolve its Mach rendezvous service because the user process remained outside the GUI bootstrap namespace. The active follow-up keeps `launchctl asuser` privileged, then drops the fixed helper to the exact console identity before direct Chrome execution while retaining process-group cleanup; it must still prove a learned route, one reload, CSS/JavaScript/image completion, DOM mutation, and one browser-ready callback. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. Reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
