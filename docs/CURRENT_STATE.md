...
`93160d357ab3f7bd791905669bb39c34309a95f5`. PR #241 added phase-specific
owned-Geph payload diagnostics with a bounded secret-redacted private-log tail.
Exact-main CI run `30308249557` and audit run `30308249444` passed, including
the packaged app lifecycle sentinel.

Protected run `30308703809` passed checkout, packaging, resource validation,
the daemon-free boundary, and final user/system cleanup, but stopped before
browser launch. The pinned Geph client reported `cannot get token: rate
limited`, followed by its overall dial/mux/auth timeout. The pinned upstream
broker implementation maintains a per-user UTC-day login counter and returns
`AuthError::RateLimited` after more than 20 token requests. This is an external
account/day qualification block, not evidence about target sites or production
routing. The harness must fail early without retrying the same protected run
until the broker day resets. Successful run `30298855867` remains the latest
complete owned-Geph payload evidence for the same production config and
recorded artifact. Protected browser completion is still open.
...
| M1 - Autonomous Routing V1 | Partial; protected packaged Chromium functional qualification is the active gate | PRs #219-#241 cover generic transport recovery, the strict semantic contract, owner-only daemon IPC, independent exact-host owned-Geph confirmation, exact Chromium-origin authentication, one bounded reload, unsigned Safari packaging, pinned Chrome for Testing, exact profile-local native-host composition, sandboxed LaunchServices bootstrap, extensionless runner-bundle adaptation, an independent HTTPS canary portfolio, phase-specific private diagnostics, and fail-closed cleanup. Exact-main CI `30308249557` and audit `30308249444` passed on `93160d3`. Protected run `30308703809` left no user/system artifact but was blocked before browser launch by the upstream broker's per-user UTC-day authentication limit. Run `30298855867` previously passed initial, trayless, and recovered payload with the same production config. Do not hot-loop the protected account; rerun once after the broker day resets. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. Reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
