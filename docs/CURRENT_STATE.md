...
[PR #214](https://github.com/aiwaki/slipstream/pull/214) at main commit
`96dd24e75b3aca75755d54b6b3a259f39418a644`. All required checks and the
packaged lifecycle passed in
[run 30160087913](https://github.com/aiwaki/slipstream/actions/runs/30160087913),
the Windows adapter qualification passed in
[run 30160087900](https://github.com/aiwaki/slipstream/actions/runs/30160087900),
and dependency plus vendored-Geph audits passed in
[run 30160087901](https://github.com/aiwaki/slipstream/actions/runs/30160087901).
...
|---|---|---|
| M0 - Safe Base | YouTube media fix qualified; workstation currently rolled back cleanly | The exact PR #214 artifact installed transactionally, became active without Geph, preserved the user's DNS and Telegram proxy, and kept ChatGPT, Discord, YouTube web, and real `yt-dlp --test` Googlevideo media reachable. A separate generic HTTP/2 partial-payload stall exposed an autonomous-recovery loop. The prepared one-shot uninstall again removed the launchd job, listener, status, PF anchor/token, and owned processes cleanly; the app bundle remains installed but the root daemon is disabled. Do not leave Slipstream active until the generic ladder patch passes exact-artifact runtime smoke. |
| M1 - Autonomous Routing V1 | Partial; generic ordered recovery under qualification | Runtime recovery, tray-independent owned Geph, browser restart, wake/network simulation, and deterministic traffic contracts exist. Local PF readiness is independent of optional Geph. Geo-exit backend loss preserves Discord/YouTube routing and falls back only to the exact pre-PF system destination, which may represent direct access, user DNS selection, a user VPN, or their combination. Generic unknown recovery is being corrected to progress through the exact system destination, app-owned Xbox DNS, then the local DoH/adaptive ladder instead of looping back after Xbox failure; no stage may select Geph. Owned-Geph cooldown and transient Keychain unavailability cannot force a Geph redial or erase opt-in state. A user full-tunnel `utun*` default route keeps Slipstream dormant and untouched; split/per-app VPN equivalence is not yet physically qualified. The protected `owned-geph-qualification` workflow has no passing run, and a physical default-route/lid-close transition on a disposable Mac is still unverified. |
