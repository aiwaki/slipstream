...
`7436d895d5044e54a13164b60b2faab6e3389ba0`. PR #239 replaced the
whole-bundle alias with a real owner-private `.app` directory whose `Contents`
entry links to the extensionless runner bundle. Exact-main CI run
`30301137419` and audit run `30301137339` passed, including the packaged app
lifecycle sentinel.

Protected run `30301572440` passed checkout, packaging, resource validation,
and the daemon-free boundary but stopped before browser launch: its single
Steam payload canary timed out through owned Geph. The failure path then exposed
an independent cleanup gap because tray startup had registered the exact
Chromium native-host manifest before the browser harness took ownership of its
cleanup. The active narrow follow-up rotates among independent real HTTPS
payload canaries and removes only a manifest that is owner-private and exactly
bound to the packaged Slipstream executable and frozen extension origin.
Foreign manifests fail closed and remain untouched. The LaunchServices wrapper
itself remains locally qualified, but protected browser completion is still
open.
...
| M1 - Autonomous Routing V1 | Partial; protected packaged Chromium functional qualification is the active gate | PRs #219-#239 cover generic transport recovery, the strict semantic contract, owner-only daemon IPC, independent exact-host owned-Geph confirmation, exact Chromium-origin authentication, one bounded reload, unsigned Safari packaging, pinned Chrome for Testing, exact profile-local native-host composition, sandboxed LaunchServices bootstrap, extensionless runner-bundle adaptation, and fail-closed cleanup. Exact-main CI `30301137419` and audit `30301137339` passed on `7436d89`. Protected run `30301572440` reached the account-backed phase but its single Steam payload canary timed out before browser launch and exposed an exact native-host cleanup gap. The active follow-up uses a bounded independent payload-canary portfolio and exact manifest cleanup; it changes neither production policy nor workstation state. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. Reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
