...
The two affected hosts produced the same bounded non-passing state in both
browsers. That is not evidence to relax the four-host/two-browser matrix,
routing, controls, deadlines, or acceptance rules, and it does not yet identify
an evidenced product or harness defect to change. A prior local browser-only
probe remains non-evidence: it did not install or run Slipstream or activate
its candidate daemon, private PF route, or owned-Geph path.

No further account-backed or readiness run may be dispatched on 2026-08-22 UTC.
The exact candidate remains current but unreleasable. After the next broker
UTC-day reset, verify that main has not changed and that no account-backed run
already happened that day; if unchanged, run at most one owned-Geph
qualification and one readiness bound to this exact candidate. If main changes,
first obtain a successful exact-main candidate. Only a completely passed matrix,
measured 30-minute soak, attestations, and exact-SHA gates may dispatch the
publisher.
