...
`033ae5af60866763a6d99287f9f2884022ec94e9`. PR #240 added the bounded
independent HTTPS canary portfolio and exact Chromium native-host cleanup.
Exact-main CI run `30304594275` and audit run `30304594228` passed, including
the packaged app lifecycle sentinel.

Protected run `30305179518` passed checkout, packaging, resource validation,
the daemon-free boundary, and final user/system cleanup, but stopped before
browser launch: every real HTTPS canary timed out through the ownership-verified
Geph listener. This is distinct from successful run `30298855867`, where the
same production config and recorded Geph artifact returned a complete
67,973-byte Steam response before and after tray/Geph restarts. Production Geph
config did not change between those commits. The active narrow follow-up reports
the exact SOCKS/TLS/HTTP timeout phase and a secret-redacted bounded tail of the
private Geph logs before cleanup. It changes neither production routing nor
workstation state. The LaunchServices wrapper itself remains locally qualified,
but protected browser completion is still open.
...
| M1 - Autonomous Routing V1 | Partial; protected packaged Chromium functional qualification is the active gate | PRs #219-#240 cover generic transport recovery, the strict semantic contract, owner-only daemon IPC, independent exact-host owned-Geph confirmation, exact Chromium-origin authentication, one bounded reload, unsigned Safari packaging, pinned Chrome for Testing, exact profile-local native-host composition, sandboxed LaunchServices bootstrap, extensionless runner-bundle adaptation, an independent HTTPS canary portfolio, and fail-closed cleanup. Exact-main CI `30304594275` and audit `30304594228` passed on `033ae5a`. Protected run `30305179518` reached the ownership-verified Geph phase and left no user/system artifact, but all three real HTTPS payload probes timed out before browser launch. Run `30298855867` had previously passed initial, trayless, and recovered payload with the same production config, so the active follow-up exposes phase-specific and secret-redacted Geph diagnostics rather than changing routing. Static policy remains authoritative; Discord/YouTube never use Geph and external network owners remain read-only. Reviewed extension distribution, signed Safari sandbox qualification, scoped QUIC, split/per-app VPN, and physical sleep/default-route qualification remain open. |
