...
Continue M4 on disposable systems. Finish PR #192 by proving the separately
owned Wintun address and exact route are both removed on native AMD64 and ARM64.
Only after that gate passes, add a disposable socket-binding fixture that proves
outbound loop avoidance with a competing capture route. It must retain the
original kernel-selected egress interface and source, bind the outbound socket
explicitly, and never use the capture interface. Fresh
