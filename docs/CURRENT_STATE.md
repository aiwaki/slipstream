...
| M1 - Autonomous Routing V1 | Exact v1 gates pass; additive incomplete-response v2 is implemented locally and awaiting qualification | PRs #219-#249 cover generic transport recovery, strict semantic qualification, owner-only daemon IPC, exact-host owned-Geph confirmation and re-admission, browser-origin authentication, bounded reload, cleanup, persistent privileged artifact attestation, Googlevideo local-only fallback, exact protected-artifact publication, and the controlled workstation rollback record. Exact-main CI `30384181967` and audit `30384182997` pass on checkpoint main `981002ea7291ffde62244a3bff0df9a65bd9009a`; protected run `30379005119` qualified the earlier exact artifact and frozen v1 browser path. The resulting workstation evidence exposed a generic partial first response after 13,672 direct bytes. Branch `codex/browser-incomplete-response-signal` adds frozen semantic-signal v2, distinct complete-response owned-Geph proof, and one same-host reload without replaying the failed request. It adds no hostname rule and leaves Discord/YouTube and external DNS/proxy/PAC/VPN/PF owners untouched. Full local, PR, exact-main, and protected two-scenario browser gates remain before v2 or its artifact may be installed. |
