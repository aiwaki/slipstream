...
Continue M4 on disposable systems. Add a disposable socket-binding fixture
that proves outbound loop avoidance with a competing capture route on native
AMD64 and ARM64. It must retain the original kernel-selected egress interface
and source, bind the outbound socket explicitly, and never use the capture
interface. Fresh
