...
3. В меню (иконка в строке меню):
   - **Geph → Account…** — вставить ключ аккаунта Geph.
   - **Geph → выбрать выход** — город или **Automatic**.

Если Telegram Desktop не подключается напрямую, появится предложение включить
встроенный Telegram-прокси Slipstream.

> [!TIP]
> Сборка не нотарифицирована Apple. Если macOS блокирует скачанное
> приложение, его можно открыть через правый клик → **Open**.
